...
#
# use Foundation;
# require '/usr/local/xhooks/lib/perlplist.pl';
#
###########################################################################
...
#    represent the type.
#
#    Containers: -d dict, -a array replace index, -ai array insert at index
#    Values: -t date, -i int, -f float, -b bool, -s string, -h hex data, -e base64 data
#
#    The value is either going to be a key (if the preceding token is -d), index (if the
#    preceding token is -a or -ai), or value (the other tokens).
#
...
#    Copies cocoaDict1, adds cocoaDict2 to it, returns the copy.
#
#  printObject()
...
sub defaultsFromString {
	...
	if ( $plist and $$plist ) {
		if ( $plist->isKindOfClass_( NSDictionary->class ) or $plist->isKindOfClass_( NSArray->class ) ) {
			...
			print STDERR "ERROR perlplist.pl, defaultsFromString: Object type was not a dictionary or array.\n";
		}
	} else {
		print STDERR "ERROR perlplist.pl, defaultsFromString: Could not convert string to plist.\n";
	}
}
...
sub saveDefaults {
	...
	if ( ( -f $file and -w $file ) or -w ( dirname "$file" ) ) {
		...
		print STDERR "ERROR perlplist.pl, saveDefaults: File $file is not writable.\n";
		return 0;
	}
}
			...
			if ( $object and $$object ) {
				if ( $object->isKindOfClass_( NSArray->class ) ) {
					...
					if ( $keyIndex < $count ) {
						...
						print STDERR "ERROR perlplist.pl, getPlistObject: Tried to get an index that doesn't exist.  Count: $count\n";
						return;
					}
				} elsif ( $object->isKindOfClass_( NSDictionary->class ) ) {
					$object = $object->objectForKey_( $keyIndex );
				} else {
					print STDERR "ERROR perlplist.pl, getPlistObject: Unknown type (not an array or a dictionary): $object\n";
					return;
				}
			} else {
				print STDERR "ERROR perlplist.pl, getPlistObject: Got nil or other error for $keyIndex in (@keysIndexes).\n";
				return;
			}
		}
	}
...
#						print "match @$path_so_far, $key\n" if $debug;
			...
			if ( $grep_pattern ne "" and $string =~ /$grep_pattern/ ) {
					if ( $#keysIndexes >= 0 ) {
						print STDERR "ERROR perlplist.pl, pathsThatMatchPartialPathLooper: There are more keys/indexes but I just searched a non container type...";
					} else {
#						print "match @$path_so_far, $string\n" if $debug;
		...
		print STDERR "ERROR perlplist.pl, pathsThatMatchPartialPathLooper: Trying to search a container that is nil.\n";
		return;
	}
}
...
sub setPlistObjectForce {
	...
	if ( ref ( $pathRef ) ne "ARRAY" ) {
		print STDERR "ERROR perlplist.pl, setPlistObjectForce: wrong path type (should be a reference)\n";
		return;
	}
	my @path = @$pathRef;
	...
	for ( my $ii = 0; $ii <= $#path; $ii+=2 ) {
		...
		if ( ! defined $do_I_exist or ! $$do_I_exist ) {
			$make_change = 1;
		...
		if ( $make_change ) {
			...
			if ( $data_type_token eq $perlplist_dict_token ) {
				...
				print STDERR "ERROR perlplist.pl, setPlistObjectForce: unknown token type! $data_type_token\n";
			}
			...
			if ( $last_container_type eq $perlplist_array_token or $last_container_type eq $perlplist_array_insert_token ) {
				if ( $parentContainer->isKindOfClass_( NSArray->class ) ) {
					...
					print STDERR "ERROR perlplist.pl, setPlistObjectForce: Tried to add array index to non-array container, stopping.\n";
					return;
				}
				...
				if ( $parentContainer->isKindOfClass_( NSDictionary->class ) ) {
					...
					print STDERR "ERROR perlplist.pl, setPlistObjectForce: Tried to add dictionary key-value pair to non-dictionary container, stopping.\n";
					return;
				}
			}
...
sub setPlistObject {
	...
	if ( $parentContainer and $$parentContainer ) {
		if ( $parentContainer->isKindOfClass_( NSArray->class ) ) {
			...
			print STDERR "ERROR perlplist.pl, setPlistObject: Unknown parent container type.\n";
		}
	} else {
		print STDERR "ERROR perlplist.pl, setPlistObject: Could not get value specified by @keyesIndexesValue.\n";
	}
}
...
sub removePlistObject {
	...
	if ( $parentContainer and $$parentContainer ) {
		if ( $parentContainer->isKindOfClass_( NSArray->class ) ) {
			...
			print STDERR "ERROR perlplist.pl, removePlistObject: Unknown parent container type.\n";
		}
	} else {
		print STDERR "ERROR perlplist.pl, removePlistObject: Could not get value specified by @keyesIndexesValue.\n";
	}
	return $success;
}
...
sub cocoaDictFromPerlHash {
	...
	while ( my ( $key, $value ) = each( %hash ) ) {
		if ( defined $value ) {
			...
			print STDERR "ERROR perlplist.pl, cocoaDictFromPerlHash: The value was not defined for $key!\n";
		}
	}
	return $cocoaDict;
}
...
sub cocoaArrayFromPerlArray {
	...
	foreach my $value ( @perlArray ) {
		if ( defined $value ) {
			...
			print STDERR "ERROR perlplist.pl, cocoaArrayFromPerlArray: The value was not defined!\n";
		}
	}
	return $cocoaArray;
}
...
sub cocoaDataFromHex {
	...
	if ( $hex !~ /^[0-9a-f]+$/ ) {
		print STDERR "ERROR perlplist.pl, cocoaDataFromHex: could not make an NSData from the following hex string: $hex (because it is not hex!)\n";
		return;
	}
...
sub cocoaDataFromBase64 {
	...
	if ( $base64 !~ /^[0-9a-zA-Z\+\/]+\=*$/ ) {
		print STDERR "ERROR perlplist.pl, cocoaDataFromBase64: could not make an NSData from the following base64 string: $base64 (because it is not base64!)\n";
		return;
	}
	$base64 =~ s/[\n\r\t ]//g;
	...
	if ( $plist and $$plist ) {
		if ( $plist->isKindOfClass_( NSData->class ) ) {
			...
			print STDERR "ERROR perlplist.pl, cocoaDataFromBase64: Object type was not nsdata.\n";
		}
	} else {
		print STDERR "ERROR perlplist.pl, cocoaDataFromBase64: Could not convert string to nsdata plist.\n";
	}
}
...
sub plistDiff {
	...
	if ( objectType( $value1 ) eq objectType( $value2 ) ) {
	...
	if ( index ( $$diff_options{'output_types'}, $result ) >= 0 ) {
		if ( $$diff_options{'output'} eq 'print' ) {
			...
			if ( $result eq "<" ) {
				...
				confess( "ERROR: A combine for > should have been dealt with elsewhere.\n" );
			} elsif ( $result eq "=" ) {
			} elsif ( $result eq "!" ) {
				confess( "ERROR: Your plist files share keys but have different values $path.\n" );
			} elsif ( $result eq "?" ) {
				confess( "ERROR: Can't combine when using a max depth (shouldn't even be possible).\n" );
			}
		} else {
			confess( "ERROR: Error, the plistDiff subroutine needs an optional 'print', 'return', or 'combine' string: plistDiff(cocoaDict1, cocoaDict2, ['print'],3); # or use 'return', the number 3 refers to the max depth." );
		}
	}
}
...
sub numberType {
	...
	if ( $type eq 'i' or $type eq 'q' ) {
		...
		confess( "ERROR: Unknown number type $type (see http://developer.apple.com/library/mac/#documentation/cocoa/Conceptual/ObjCRuntimeGuide/Articles/ocrtTypeEncodings.html)\n" );
	}
}
...
sub objectType {
	...
	if ( ! defined $value or $value eq "" ) {
		...
		confess( "ERROR: Uninitialized value in objectType. Package $package, in file $filename, line $line.\n" );
	}
	if ( $value =~ /^(__)?NS(CF)?Array/ ) {
		...
		confess( "ERROR: Unknown Objective-C object type: ". ref ( $value ) . " value: $value" );
	}
}
...
sub combineObjects {
	...
	my $result;
	if ( objectType ( $value1 ) eq "NSCFArray" or objectType ( $value1 ) eq "NSCFDictionary" ) {
		$result = $value1->mutableCopy;
	} else {
		$result = $value1;
	}
	$diff_options = {} if ! defined $diff_options;
	$$diff_options{'path'} = '' if ! defined $$diff_options{'path'}; # path
	if ( objectType( $result ) eq objectType( $value2 ) ) {
		if ( objectType ( $result ) eq "NSCFArray" ) {
			print "array\n" if $debug;
			combineLoopThroughNSArrays( $result, $value2 );
		} elsif ( objectType ( $result ) eq "NSCFDictionary" ) {
			print "dictionary\n" if $debug;
			combineLoopThroughNSDicts( $result, $value2, $diff_options );
		} else {
			print "\nvalue2 ($value2 ".perlValue($value2).") has precendence over result ($result ".perlValue($result).").\n" if $debug;
			$result = $value2;
		}
	} else {
		print "\nobject2 ($value2 ".perlValue($value2).") has precendence over object1 ($result ".perlValue($result).").\n" if $debug;
		$result = $value2;
	}
	return $result;
}

sub combineLoopThroughNSArrays {
	my ( $cocoaArray1, $cocoaArray2 ) = ( @_ );
	print "sub combineLoopThroughNSArrays start line " . __LINE__ . "\n" if $debug and $debug_tracing;
	my $enumerator2 = $cocoaArray2->objectEnumerator();
	my $value2;
	my $counter = 0;
	# Add all objects in the second array that isn't present in the first array.
	while ( $value2 = $enumerator2->nextObject() and $$value2 ) {
		my $present = $cocoaArray1->containsObject_( $value2 );
		if ( ! $present ) {
			print perlValue($value2)." is missing from the first array, adding it.\n" if $debug;
			$cocoaArray1->addObject_( $value2 );
		} elsif ( $debug ) {
			print perlValue($value2)." is present in the first array.\n" if $debug;
		}
	}
}

sub combineLoopThroughNSDicts {
	my ( $cocoaDict1, $cocoaDict2, $diff_options ) = ( @_ );
	print "sub combineLoopThroughNSDicts start line " . __LINE__ . "\n" if $debug and $debug_tracing;
	my $path = $$diff_options{'path'};
	my $enumerator1 = $cocoaDict1->keyEnumerator();
	my $key1;
	my %found_keys = ();
	my %addToDict1 = ();
	# Record all found keys in the first array and if the key is in the second array compare the values.
	while ( $key1 = $enumerator1->nextObject() and $$key1 ) {
		my $value1 = $cocoaDict1->objectForKey_($key1);
		if ( $value1 ) { # check to make sure an object was set
			$found_keys{perlValue($key1)} = '1';
			...
			if ( $$value2) { # check to make sure an object was set
				$$diff_options{'path'} = "$path/{".perlValue($key1)."}";
				print $$diff_options{'path'} ."\n" if $debug;
				$addToDict1{perlValue($key1)} = combineObjects( $value1, $value2, $diff_options );
			}
		}
	}
	while ( my ( $key, $value ) = each %addToDict1 ) {
		$cocoaDict1->setObject_forKey_($value, $key);
	}
	# Find keys present in the second array but not in the first.
	my $enumerator2 = $cocoaDict2->keyEnumerator();
	my $key2;
	while ( $key2 = $enumerator2->nextObject() and $$key2 ) {
		my $value2 = $cocoaDict2->objectForKey_($key2);
...
sub prettyPrintObject {
	...
	if ( $$value and $value ) {
		...
		if ( objectType ( $value ) eq "NSCFArray" ) {
			...
			confess( "ERROR: Unknown Objective-C object type: ". ref ( $value ) );
		}
		print $text;
		$total_text .= $text;
	} else {
...
sub dataToASCII {
	my ( $nsdata_obj ) = @_;
	my $hex_string = $nsdata_obj->description()->UTF8String();
	$hex_string =~ s/ //g;
	$hex_string =~ s/<(([0-9a-f][0-9a-f])+)>/pack('H*', $1)/ie;
	return $hex_string;
}
